...
import services.payments, services.mining, services.support
from utils import get_translator

from handlers.balance import balance_handler
from handlers.buy import buy_start, buy_confirm
from handlers.refer import refer_handler
...
async def language_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("Select language / Выберите язык:", reply_markup=language_kb())

async def lang_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    code = update.callback_query.data.split('_')[1]
    user = update.effective_user
    u, _ = models.User.get_or_create(user.id)
    u.language = code; u.save()
...
async def support_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(_("Опишите вашу проблему:"), reply_markup=InlineKeyboardMarkup([
...
async def support_receive(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(chat_id=config.SUPPORT_CHAT_ID, text=f"#{req.id} {user.full_name}: {text}")
    _ = get_translator(models.User.get_or_create(user.id)[0].language)
...
async def set_report_time(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user = update.effective_user; u, _ = models.User.get_or_create(user.id)
    _ = get_translator(u.language)
    ...
    context.job_queue.run_daily(daily_report, time(hour=hh, minute=mm), context=user.id, name=str(user.id))
    await update.message.reply_text(_("Отчёт будет приходить каждый день в {:02d}:{:02d}").format(hh, mm))

async def daily_report(context: ContextTypes.DEFAULT_TYPE):
    user_id = context.job.context; u = models.User[user_id]
    _ = get_translator(u.language); report = services.mining.get_daily_report(u)
...
def main():
    ...
    conv = ConversationHandler(entry_points=[CommandHandler('support', support_start)],
    ...
    for u in models.User.with_report_time():
        hh, mm = u.report_time
        app.job_queue.run_daily(daily_report, time(hour=hh, minute=mm), context=u.id, name=str(u.id))
